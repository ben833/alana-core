...
var intentLocation = process.env.INTENT_LOCATION ? process.env.INTENT_LOCATION : path.dirname(require.resolve('botler-intents'));
var locations = loadLocations(intentLocation + "/locations");
var dateClassifiers = classifier_1.GenerateClassifier([intentLocation + "/dates"]);
...
function loadLocations(theDirectory) {
    ...
    fs.readdirSync(theDirectory)
        ...
        fs.readdirSync(theDirectory + "/" + topic)
            ...
            var contents = fs.readFileSync(theDirectory + "/" + topic + "/" + file, 'utf8');
            var phrases = JSON.parse(contents);
