...
var Script = (function () {
    ...
    Script.prototype.run = function (incoming, outgoing, nextScript, step, skipAlways) {
        var _this = this;
        if (step === void 0) { step = incoming.user.scriptStage; }
        if (skipAlways === void 0) { skipAlways = false; }
            ...
            if (err instanceof GotoDialogException) {
                for (var i = 0; i < _this.dialogs.length; i++) {
                    ...
                    if (dialog instanceof dialog_1.default && dialog.name === err.dialogName) {
                        ...
                        return _this.run(request, response, nextScript, i, true);
                    }
                }
            }
